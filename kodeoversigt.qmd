...
I forlæns modelsøgning undersøger man forholdet mellem en afhængig variabel Y og en uafhængig variabel X. Man bruger regressionsanalyse til at teste sammenhængen mellem disse to variable, imens man gradvist inkluderer flere og flere kontrolvariable. I dette eksempel undersøges sammenhængen mellem indkomst (månedlig netto indkomst) og antal års uddannelse.

### Simpel model

Man kan bruge funktionen `lm()` til at estimere en simpel model med kun afhængig (indkomst) og én uafhængig variabel (antal års uddannelse):
...
Regressionstabellen indeholder følgende oplysninger:
...
ggsave("plot_heteroskedasticitet.jpeg", plot = plot_heteroskedasticitet, width = 10, height = 8)
```
...
**H0**: Der er *ingen* heteroskedasticitet i residualerne

**H1**: Der *er* heteroskedasticitet i residualerne
